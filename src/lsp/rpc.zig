...
pub const BaseMessage = struct {
    ...
    pub fn readMessage(allocator: std.mem.Allocator, stdin: *std.Io.Reader) !std.json.Parsed(BaseMessage) {
        try stdin.discardAll(16); // skip "Content-Length: "

        // get the length of the message in the header
        const buf = try stdin.takeDelimiterExclusive('\r');
        const msg_size = try std.fmt.parseInt(u32, buf, 10);

        // skip newlines "\r\n\r\n"
        try stdin.discardAll(3);

        const contents = try allocator.alloc(u8, msg_size);
        try stdin.readSliceAll(contents);
...
pub fn readMessage(allocator: std.mem.Allocator, base_message: *const BaseMessage, T: type) !std.json.Parsed(T) {
    var stream = std.Io.fixedBufferStream(base_message.contents.?);
    var stdin = stream.reader();
...
test "test BaseMessage.readMessage" {
    ...
    const json =
        ...
        // \\{"clientInfo":{"name":"Neovim", "version":"0.10.2"}}
    ;
    ...
    var reader = f.reader(&b);

    const parsed = try BaseMessage.readMessage(allocator, &reader.interface);
    defer parsed.deinit();
...
test "test readMessage" {
    ...
    const message = parsed.value;
    defer message.deinit(allocator);
    const parsed_message = try readMessage(allocator, &message, structs.RequestMessage(structs.InitializeParams));
    defer parsed_message.deinit();
    const val = parsed_message.value;
    try std.testing.expectEqualStrings("Neovim", val.params.?.clientInfo.name);
    try std.testing.expectEqualStrings("0.10.2", val.params.?.clientInfo.version);
}
